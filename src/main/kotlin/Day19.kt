...
fun main() {
    val input = readFileAsList("Day19")
...
object Day19 {
    fun part1(input: List<String>): Int {
        val blueprints = input.map(Blueprint::from)
        return blueprints.sumOf { it.id * Factory(it, minutes = 24).findBest() }
    }

    fun part2(input: List<String>): Int {
        val blueprints = input.map(Blueprint::from)
        return blueprints.take(3).map { Factory(it, minutes = 32).findBest() }.reduce(Int::times)
    }
